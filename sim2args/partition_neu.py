...
helpMsg = '''
        usage: $./partition_neu.py <pkl_path> <out_pref> <no_threads>
...
def main(args):
    ...
    for thread in range(1, no_threads+1):
        ...
        with open(out_pref+'/'+out_pref+'_pgv_'+str(thread)+'.pickle', 'wb') as f:
            pickle.dump((list_pos[a_idx:b_idx], list_geno[a_idx:b_idx], list_variant[a_idx:b_idx], list_variant_pos[a_idx:b_idx]), f, pickle.HIGHEST_PROTOCOL)
