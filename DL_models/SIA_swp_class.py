## Training script for classifying sweeps
...
model.add(Dense(np.shape(trainY)[1], activation=activation))
...
output = "C_" + architecture + str(nepoch) + "_predictions.txt"
np.savetxt(output, pred, fmt = '%f', delimiter=' ')
output = "C_" + architecture + str(nepoch) + "_truth.txt"
np.savetxt(output, testY, fmt = '%f', delimiter=' ')

model_name = 'C_' + architecture + ".h5"
